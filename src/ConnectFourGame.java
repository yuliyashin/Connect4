...
public class ConnectFourGame extends JFrame {
	...
	private int ROW; // default 13 is max
	private int COL; // default
	...
	final String winningSoundFilePath = "yaySound.wav";
	final String MusicFilePath = "backgroundMusic.wav";
//	final String popSound = "popSound.wav";
	final String losingSoundFilePath = "ohNoSound.wav";

	PlayAudio backgroundMusic;
	PlayAudio winningSound = new PlayAudio();
	...
	ConnectFourGame() {
		try {
//			backgroundMusic = new PlayAudio();
//			backgroundMusic.doPlay(MusicFilePath);
			setTitle("Connect 4!");
	...
	private void boardSizeOption() throws NumberFormatException {
		...
		String row = JOptionPane
				.showInputDialog(null, enterRow, "User input", JOptionPane.PLAIN_MESSAGE, gameBoard, null, "")
				.toString();
		String col = JOptionPane
				.showInputDialog(null, enterCol, "User input", JOptionPane.PLAIN_MESSAGE, gameBoard, null, "")
				.toString();
	...
	private int gameModeOption() {
		...
		int choice = JOptionPane.showOptionDialog(this, chooseGameMode, "Select a game mode", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, versus, buttonArray, buttonArray[0]);
		return choice;
	}
	...
	private boolean checkIfWon(int row, int col) {
		return (checkHorizontal(row, col) || checkVertical(row, col) || checkBottomLeftToTopRight(row, col)
				...
				for (int i = (ROW - 1); i >= 0; i--) { // checks for empty space in selected column
					...
					if (checkedSpot.getIcon() == emptyCircle) {
						...
						if (activePlayer == PLAYER_ONE) {
							...
							if (playerWon) {
								if (soundOn) {
									winningSound.doPlay(winningSoundFilePath);
								}
								P1Score++;
								player1Score.setText(String.valueOf(P1Score));

								int option = JOptionPane.showConfirmDialog(null,
										"Player 1 Wins! Would you like to play again?", "* We have a winner! *",
							...
							if (itsADraw) {
								int option = JOptionPane.showConfirmDialog(null,
										"Nobody Wins! Would you like to play again?", "* It's a draw! *",
										JOptionPane.YES_NO_OPTION);
							...
							if (playerWon) {
								...
								int option = JOptionPane.showConfirmDialog(null,
										"Player 2 Wins! Would you like to play again?", "* We have a winner *",
										JOptionPane.YES_NO_OPTION);
								if (option == JOptionPane.YES_OPTION) {
									resetBoard();
								} else {
	...
	private void aiMove() {
		...
		if (playerWon) {
			...
			int option = JOptionPane.showConfirmDialog(null, "AI Wins! Would you like to play again?",
		...
		if (itsADraw) {
			int option = JOptionPane.showConfirmDialog(null, "Nobody Wins! Would you like to play again?",
					"* It's a draw! *", JOptionPane.YES_NO_OPTION);
	...
	class ButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == reset) {
				int confirmed = JOptionPane.showConfirmDialog(null, "Are you sure you want to reset the game?",
